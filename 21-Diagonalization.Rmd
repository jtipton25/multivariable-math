...
```{proof}
This comes directly from \@ref(thm:distinct-eigenvalues) where if a $n \times n$ matrix has $n$ distinct eigenvalues $\lambda_1 \neq \lambda_2 \neq \cdots \neq \lambda_n$, the the corresponding eigenvalues $\mathbf{v}_1, \mathbf{v}_2, \ldots, \mathbf{v}_n$ are linearly independent.
```
...
\begin{align*}
`r array_to_latex(A)` & = `r array_to_latex(matrix(c(0, 0, 1, 0, 1, 0, 1, 0, 0), 3, 3))` `r array_to_latex(diag(c(3, 2, 1)))` `r `r array_to_latex(matrix(c(0, 0, 1, 0, 1, 0, 1, 0, 0), 3, 3))``^{-1}
\end{align*}
