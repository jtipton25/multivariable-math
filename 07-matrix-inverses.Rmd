...
**Theorem:** Let $\mathbf{A} = \begin{pmatrix} a & b \\ c & d \end{pmatrix}$. If $ad - bc \neq 0$ then $\mathbf{A}$ is invertible and 
$$
\begin{align*}
...
**Definition:** For the $2 \times 2$ matrix $\mathbf{A} = \begin{pmatrix} a & b \\ c & d \end{pmatrix}$, the term $ad - bc$ is called the **determinant** of the matrix $\mathbf{A}$ and is written as $\operatorname{det}(\mathbf{A})$. Sometimes the determinant is written as $| \mathbf{A}|$
...
**Theorem:** If the $n \times n$ matrix $\mathbf{A}$ is invertible, then for each $\mathbf{b} \in \mathcal{R}^n$, the matrix equation
$$
...
***Note:** The product of $k$ invertible $n \times n$ matrices $\mathbf{A}_1 \mathbf{A}_2 \cdots \mathbf{A}_k$ has inverse $\mathbf{A}_k^{-1} \mathbf{A}_{k-1}^{-1} \cdots \mathbf{A}_1$
...
**Theorem:** If an $n \times n$ matrix $\mathbf{A}$ is invertible, then $\mathbf{A}$ is row-equivalent to $\mathbf{I}$ ($\mathbf{A} \sim \mathbf{I}$; row-equivalent means $\mathbf{A}$ can be reduced to $\mathbf{I}$ using elementary row operations). The row-equivalency implies that there is a series of elementary row operations (e.g., elementary matrices $\mathbf{E}_1, \ldots, \mathbf{E}_k$) that converts $\mathbf{A}$ to $\mathbf{I}$. In addition, the application of these row matrices to $\mathbf{I}$ transforms $\mathbf{I}$ to the matrix inverse $\mathbf{A}^{-1}$.
...
**The Invertible Matrix Theorem:** Let $\mathbf{A}$ be an $n \times n$ matrix. Then the following statements are equivalent (i.e., they are all either simultaneously true or false).
...
**Definition:** A linear transformation $T:\mathcal{R}^n \rightarrow \mathcal{R}^n$ is said to be invertible if there exists a transformation $S:\mathcal{R}^n \rightarrow \mathcal{R}^n$ such that
