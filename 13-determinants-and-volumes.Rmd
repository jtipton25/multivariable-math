...
::: {.definition}
The **parallelpiped** defined by $n$ vectors $\mathbf{a}_1, \mathbf{a}_n, \ldots, \mathbf{a}_n \in \mathcal{R}^n$ with coefficients $x_1, x_2, \ldots, x_n$ is the subset
...
library(shiny)
runGitHub(rep = "multivariable-math", username = "jtipton25", subdir = "shiny-apps/chapter-14/determinants-volume/") 
```
...
\begin{align*}
|\det(\mathbf{A})| = \operatorname{volume}(\mathcal)
\end{align*}
```
<!-- ::: -->

::: {.proof}
Recall the 4 defining properties (**link to definition**) that characterize the determinant. These properties also characterize the absolute value of the determinant.
...
```{example}
2 by 2 matrix $a  b c d$ **in class**
```
...
\begin{align*}
f(\mathbf{y}) = \frac{1}{\sqrt{2 \pi}} |\det(\boldsymbol{\Sigma})|^{-1/2} e^{- \frac{1}{2} (\mathbf{y} - \boldsymbol{\mu})' \boldsymbol{\Sigma}^{-1} (\mathbf{y} - \boldsymbol{\mu})}
\end{align*}
...
\begin{align*}
\int_{\mathbf{y}} \frac{1}{\sqrt{2 \pi}} e^{- \frac{1}{2} (\mathbf{y} - \boldsymbol{\mu})' \boldsymbol{\Sigma}^{-1} (\mathbf{y} - \boldsymbol{\mu})} \, d\mathbf{y} = |\det(\boldsymbol{\Sigma})|^{-1/2}
\end{align*}
which implies that 
\begin{align*}
\frac{\int_{\mathbf{y}} \frac{1}{\sqrt{2 \pi}} e^{- \frac{1}{2} (\mathbf{y} - \boldsymbol{\mu})' \boldsymbol{\Sigma}^{-1} (\mathbf{y} - \boldsymbol{\mu})} \, d\mathbf{y}}{|\det(\boldsymbol{\Sigma})|^{-1/2}} = 1
\end{align*}
