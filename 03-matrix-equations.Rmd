...
\end{align}
$$ 
is called a homogeneous system of equations. The vector $\mathbf{0}$ is a vector of length $K$ composed of all zeros. The **trivial** solution of the homogeneous equation is when $\mathbf{x} = \mathbf{0}$ and is not a very useful solution. Typically one is interested in **nontrivial** solutions where $\mathbf{x} \neq \mathbf{0}$.
...
\end{align*}
$$
which implies that a non-trivial solution only exists if there is a free variable. Another way of saying this is that at least one column must not be a pivot column. If every column were a pivot column, the reduced row echelon form of the augmented matrix would be 
$$
\begin{align*}
\begin{pmatrix}
1 & 0 & \ldots & 0 & 0 \\
...
**Definition:** A solution set of the form  $\mathbf{x} = c \mathbf{u} + d \mathbf{v}$ is called a **parametric vector solution**.
...
\end{align*}
$$
but change this so that $\mathbf{b} = \begin{pmatrix} 4 \\ -2 \\ 1 \end{pmatrix}$
...
\end{align*}
$$
we get the homogeneous solution set $x_1 = -2 x_2 + \frac{1}{2} x_3 + 4$ which can be written in parametric form as $\mathbf{x} = c \mathbf{u} + d \mathbf{v} + \mathbf{p}$$ with
$$
\begin{align*}
\mathbf{u} & = \begin{pmatrix} -2 \\ 1 \\ 0 \end{pmatrix} + 
\mathbf{v} & = \begin{pmatrix} \frac{1}{2} \\ 0 \\ 1 \end{pmatrix} \\
\mathbf{p} & = \begin{pmatrix} 20 \\ 0 \\ 0 \end{pmatrix}
\end{align*}
$$
...
**Don't know why this figure isn't working**
...
df <- data.frame(
    x1 = region$x1,
    x2 = region$x2,
    x3 = 2 * region$x1 + 4 * region$x2
)
df2 <- data.frame(
    x1 = region$x1,
    x2 = region$x2,
    x3 = 2 * region$x1 + 4 * region$x2 - 40
)
...
ggplot(df, aes(x = x1, y = x2, z = x3)) +
    ...
    stat_wireframe(
        ...
        theta = theta, phi = phi) +
    stat_wireframe(
        data = df2, aes(x = x1, y = x2, z = x3),
        alpha = 0.5, color = "blue", theta = theta, phi = phi) +
    theme_void() +
