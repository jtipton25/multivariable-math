...
:::{.example}
In class using standard basis in 2-dimensions and vector (3, 2)
:::


:::{.example}
In class using made up basis in 2-dimensions $b_1 = (1, 0)$ and $b_2 = (0.5, 1))
:::
...
:::{.proof}
* Take two vectors $\mathbf{u} = c_1 \mathbf{b}_1 + \ldots + c_n \mathbf{b}_n$ and $\mathbf{v} = d_1 \mathbf{b}_1 + \ldots + d_n \mathbf{b}_n$
* $\mathbf{u} + \mathbf{v} \rightarrow \mathbf{P}_B^{-1} (\mathbf{u} + \mathbf{v}) =  \mathbf{P}_B^{-1} \mathbf{u} + \mathbf{P}_B^{-1} \mathbf{v}$ which preserves vector addition
* Given scalar $a$, $a\mathbf{u} \rightarrow \mathbf{P}_B^{-1} (a \mathbf{u}) =  a \mathbf{P}_B^{-1} \mathbf{u}$ which preserves scalar multiplication. 
...
\begin{align*}
c_1 \mathbf{P}_B \mathbf{u}_1 + \ldots + c_n \mathbf{P}_B \mathbf{u}_n = \mathbf{0}.
\end{align*}
which by linearity implies 
\begin{align*}
\mathbf{P}_B (c_1 \mathbf{u}_1 + \ldots + c_n \mathbf{u}_n) = \mathbf{0} 
\end{align*}

:::

Thus, we know that for a vector space $\mathcal{V}$ that has a basis $\basis$ that consists on $n$ vectors, then every linearly independent set of vectors in $\mathcal{V}$ contains at most $n$ vectors. 
